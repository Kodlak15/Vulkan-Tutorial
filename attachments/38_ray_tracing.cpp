...
class VulkanRaytracingApplication {
  ...
  private:
    ...
    void pickPhysicalDevice() {
        ...
        const auto devIter = std::ranges::find_if(devices, [&](auto const& device) {
            ...
            bool supportsRequiredFeatures =
                ...
                features.template get<vk::PhysicalDeviceAccelerationStructureFeaturesKHR>()
    ...
    void createLogicalDevice() {
        ...
        auto presentIndex =
            physicalDevice.getSurfaceSupportKHR(graphicsIndex, *surface) ? graphicsIndex : ~0;
        if (presentIndex == queueFamilyProperties.size()) {
            // the graphicsIndex doesn't support present -> look for another family index that
    ...
    void loadModel() {
        ...
        if (!LoadObj(&attrib, &shapes, &localMaterials, &warn, &err, MODEL_PATH.c_str(),
        ...
        for (const auto& shape : shapes) {
            std::cout << "Loading mesh: " << shape.name << ": " << shape.mesh.indices.size() / 3
                      << " triangles\n";
            ...
            int globalMaterialID =
                (localMaterialID < 0) ? -1 : static_cast<int>(materialOffset + localMaterialID);
        ...
        for (size_t i = 0; i < localMaterials.size(); ++i) {
            ...
            if (!material.diffuse_texname.empty()) {
                std::string texturePath = MODEL_PATH.substr(0, MODEL_PATH.find_last_of("/\\")) +
    ...
    void createAccelerationStructures() {
        ...
        identity.matrix = std::array<std::array<float, 4>, 3>{
            {std::array<float, 4>{1.f, 0.f, 0.f, 0.f}, std::array<float, 4>{0.f, 1.f, 0.f, 0.f},
        ...
        for (size_t i = 0; i < submeshes.size(); ++i) {
            ...
            // TASK07
            blasGeometry.flags =
                (submesh.alphaCut) ? vk::GeometryFlagsKHR(0) : vk::GeometryFlagBitsKHR::eOpaque;
            ...
            vk::AccelerationStructureBuildSizesInfoKHR blasBuildSizes =
                device.getAccelerationStructureBuildSizesKHR(
                    vk::AccelerationStructureBuildTypeKHR::eDevice, blasBuildGeometryInfo,
                    {primitiveCount});
            ...
            createBuffer(blasBuildSizes.accelerationStructureSize,
                         vk::BufferUsageFlagBits::eAccelerationStructureStorageKHR |
                             vk::BufferUsageFlagBits::eShaderDeviceAddress |
                             vk::BufferUsageFlagBits::eAccelerationStructureBuildInputReadOnlyKHR,
                         vk::MemoryPropertyFlagBits::eDeviceLocal, blasBuffers[i], blasMemories[i]);
            ...
            vk::AccelerationStructureDeviceAddressInfoKHR addrInfo{.accelerationStructure =
                                                                       *blasHandles[i]};
            vk::DeviceAddress blasDeviceAddr = device.getAccelerationStructureAddressKHR(addrInfo);

            vk::AccelerationStructureInstanceKHR instance{.transform = identity,
                                                          .mask      = 0xFF,
        ...
        vk::AccelerationStructureBuildSizesInfoKHR tlasBuildSizes =
            device.getAccelerationStructureBuildSizesKHR(
                vk::AccelerationStructureBuildTypeKHR::eDevice, tlasBuildGeometryInfo,
                {primitiveCount});
        ...
        vk::AccelerationStructureBuildRangeInfoKHR tlasRangeInfo{.primitiveCount  = primitiveCount,
                                                                 .primitiveOffset = 0,
                                                                 .firstVertex     = 0,
                                                                 .transformOffset = 0};
    ...
    void createDescriptorPool() {
        std::array poolSize{
            ...
            vk::DescriptorPoolSize(vk::DescriptorType::eStorageBuffer,
                                   MAX_FRAMES_IN_FLIGHT * 3), // indices, UVs, instance LUT
    ...
    void createDescriptorSets() {
        ...
        std::vector<vk::DescriptorSetLayout> globalLayouts(MAX_FRAMES_IN_FLIGHT,
        ...
        for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
            ...
            vk::WriteDescriptorSet asWrite{.pNext           = &asInfo,
            ...
            vk::DescriptorBufferInfo indexBufferInfo{
                .buffer = indexBuffer, .offset = 0, .range = sizeof(uint32_t) * indices.size()};

            vk::WriteDescriptorSet indexBufferWrite{.dstSet          = globalDescriptorSets[i],
                                                    .dstBinding      = 2,
                                                    .dstArrayElement = 0,
                                                    .descriptorCount = 1,
                                                    .descriptorType =
            ...
            vk::DescriptorBufferInfo uvBufferInfo{
                .buffer = uvBuffer, .offset = 0, .range = sizeof(glm::vec2) * vertices.size()};

            vk::WriteDescriptorSet uvBufferWrite{.dstSet          = globalDescriptorSets[i],
                                                 .dstBinding      = 3,
                                                 .dstArrayElement = 0,
                                                 .descriptorCount = 1,
                                                 .descriptorType =
            ...
            vk::DescriptorBufferInfo instanceLUTBufferInfo{.buffer = instanceLUTBuffer,
                                                           .offset = 0,
                                                           .range  = sizeof(InstanceLUT) *
                                                                    instanceLUTs.size()};

            vk::WriteDescriptorSet instanceLUTBufferWrite{.dstSet     = globalDescriptorSets[i],
                                                          .dstBinding = 4,
                                                          .dstArrayElement = 0,
                                                          .descriptorCount = 1,
                                                          .descriptorType =
            ...
            std::array<vk::WriteDescriptorSet, 4> descriptorWrites{bufferWrite, asWrite,
                                                                   indexBufferWrite, uvBufferWrite};
#else
            std::array<vk::WriteDescriptorSet, 3> descriptorWrites{bufferWrite, indexBufferWrite,
                                                                   uvBufferWrite};
#endif
        ...
        vk::DescriptorSetAllocateInfo allocInfo{.pNext          = &variableCountInfo,
                                                .descriptorPool = descriptorPool,
                                                .descriptorSetCount =
        ...
        for (auto& iv : textureImageViews) {
            vk::DescriptorImageInfo imageInfo{
                .imageView = iv, .imageLayout = vk::ImageLayout::eShaderReadOnlyOptimal};
            imageInfos.push_back(imageInfo);
        }

        vk::WriteDescriptorSet materialWrite{.dstSet          = materialDescriptorSets[0],
                                             .dstBinding      = 1,
                                             .dstArrayElement = 0,
                                             .descriptorCount =
    ...
    void updateUniformBuffer(uint32_t currentImage) {
        ...
        ubo.model =
            rotate(glm::mat4(1.0f), time * 0.1f * glm::radians(90.0f), glm::vec3(0.0f, 0.0f, 1.0f));
